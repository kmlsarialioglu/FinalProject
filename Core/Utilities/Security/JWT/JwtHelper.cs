...
namespace Core.Utilities.Security.JWT
{
    public class JwtHelper : ITokenHelper
    {
        ...
        private IEnumerable<Claim> SetClaims(User user, List<OperationClaim> operationClaims) // Claimleri set edebileceğimiz operasyon
        {
            ...
            claims.AddNameIdentifier(user.UserId.ToString()); // "AddNameIdentifier" parametresi string olduğu için user.Id.ToString()
            claims.AddEmail(user.Email);
            claims.AddName($"{user.FirstName} {user.LastName}");
            claims.AddRoles(operationClaims.Select(c => c.OperationClaimName).ToArray()); //"AddRoles" parametresi Array olduğu için operationClaims.Select(c => c.Name).ToArray()
